/*
 * @Author: mikey.sunyue 
 * @Date: 2018-12-12 10:12:02 
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2018-12-12 18:55:24
 */
/**
 * 听歌识曲页面
 */
...
import styles from './listenPage.css'
  ...
  render(){
    ...
    return <React.Fragment>
      <div className='song_what_wraper'>
          <h2 className='song_what'>歌曲猜猜猜</h2>
          ...
          <div>
            ...
            <div className='progress'>
              <p ref="progress">
                <span style={{width:this.state.progress+'%'}} className='progress_run'></span>
              </p>
            </div>
            <span>00.10</span>
          </div>
          ...
          <ul className={styles.list}>
            ...
            {this.state.answers.map((item, index)=>{
              return <li>
                <span>{index+1}</span>
                <span>答案：{item}</span>
                <span className='current'>{item == this.state.rightAnser[index]?'猜对':'猜错'}</span>
              </li>
            })}
          </ul>
      </div>
